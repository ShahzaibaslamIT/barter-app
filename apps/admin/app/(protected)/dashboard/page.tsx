...
const QUICK_LINKS = [
  { label: "Review Pending", href: "/listings?status=pending", color: "text-amber-600 bg-amber-50 border-amber-200 hover:bg-amber-100" },
  { label: "Open Reports", href: "/reports?status=open", color: "text-rose-600 bg-rose-50 border-rose-200 hover:bg-rose-100" },
  { label: "Manage Users", href: "/users", color: "text-blue-600 bg-blue-50 border-blue-200 hover:bg-blue-100" },
  { label: "Audit Logs", href: "/audit-logs", color: "text-slate-600 bg-slate-50 border-slate-200 hover:bg-slate-100" },
];
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch("/api/dashboard")
      ...
      .then(setStats)
      .catch((s) => { if (s === 401) router.replace("/login"); })
