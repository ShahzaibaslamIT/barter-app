...
//   // ✅ Fetch thread with no caching
//   const fetchThread = async (initial: boolean = false) => {
//     try {
//       const token = localStorage.getItem("auth_token");
//       const response = await fetch(`/api/messages/threads/${threadId}`, {
//         headers: { Authorization: `Bearer ${token}` },
//         cache: "no-store", // 🚀 ensures no stale cache
//       });
...
//           (msg: Message) =>
//             !messages.some((m) => m.message_id === msg.message_id)
//         );
...
//             )
//           );
//           setLastMessageId(newMsgs[newMsgs.length - 1].message_id);
//         }
//       }
//     } catch (err) {
...
//   // ✅ Send message immediately in UI, then sync
//   const sendMessage = async (e: React.FormEvent) => {
//     e.preventDefault();
//     if (!newMessage.trim() || isSending) return;
...
//         },
//         cache: "no-store",
//         body: JSON.stringify({ thread_id: threadId, content: msgContent }),
//       });
...
//             (m) => m.message_id !== tempMsg.message_id
//           );
...
//       ? {
//           id: thread.user_2_id,
//           name: thread.user_2_name,
//           avatar: thread.user_2_avatar,
//         }
//       : {
//           id: thread.user_1_id,
//           name: thread.user_1_name,
//           avatar: thread.user_1_avatar,
//         };
...
export function ChatInterface({ threadId, currentUserId }: ChatInterfaceProps) {
  ...
  // Polling for messages + typing indicator
  useEffect(() => {
    if (isSending) return;
    ...
    return () => clearInterval(interval);
  }, [threadId, lastMessageId, isSending]);
  ...
  // ✅ Fetch thread and normalize messages
  const fetchThread = async (initial = false) => {
    try {
      const token = localStorage.getItem("auth_token");
      const response = await fetch(`/api/messages/threads/${threadId}`, {
        headers: { Authorization: `Bearer ${token}` },
      ...
      if (Array.isArray(data.messages)) {
        ...
        const normalized = data.messages.map((msg: any) => ({
          ...
          sender_id: msg.sender_id || msg.sender?.user_id?.toString(),
          sender_name: msg.sender_name || msg.sender?.username || "Unknown",
          sender_avatar: msg.sender_avatar || msg.sender?.avatar_url || null,
          content: msg.content,
        ...
        setMessages((prev) => {
          ...
          const unique = new Map<string | number, Message>();
          for (const m of combined) {
            if (!unique.has(m.message_id)) unique.set(m.message_id, m);
          }
        ...
        if (normalized.length > 0) {
          setLastMessageId(
            normalized[normalized.length - 1].message_id as number
          );
        }
      }
    } catch (err) {
  ...
  // ✅ Check if the other user is typing (localStorage-based)
  const checkTypingStatus = () => {
    const typingData = localStorage.getItem(`typing_${threadId}`);
    if (!typingData) return setIsOtherTyping(false);

    const { userId, timestamp } = JSON.parse(typingData);
    const now = Date.now();
    // Show typing if within last 5s and not current user
  ...
  const handleTyping = (value: string) => {
    ...
    typingTimeout.current = setTimeout(() => {
      localStorage.removeItem(`typing_${threadId}`);
    }, 5000);
  };
      ...
      if (response.ok && data.message) {
        const newMsg: Message = {
          ...
          sender_id:
            ...
            data.message.sender?.user_id?.toString() ||
            currentUserId,
          sender_name:
          ...
          sender_avatar:
            ...
            data.message.sender?.avatar_url ||
            null,
        ...
        setMessages((prev) => {
          ...
          const unique = new Map<string | number, Message>();
          for (const msg of updated) unique.set(msg.message_id, msg);
          return Array.from(unique.values()).sort(
            (a, b) =>
              new Date(a.created_at).getTime() - new Date(b.created_at).getTime()
          );
        });
        setLastMessageId(newMsg.message_id as number);
      } else {
        throw new Error(data.error || "Failed to send");
      }
    } catch (err) {
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 ? (
          ...
          messages.map((message) => {
            ...
            return (
              <div
                key={`${message.message_id}-${message.sender_id}`}
                className={`flex gap-3 ${isOwn ? "flex-row-reverse" : ""}`}
              >
                <Avatar className="h-8 w-8 flex-shrink-0">
                  <AvatarImage
        ...
        {isOtherTyping && (
          <div className="pl-2 text-sm text-muted-foreground italic">
