...
//   useEffect(() => {
//     if (offer.status === "completed") checkRatingStatus();
//   }, [offer.offer_id, offer.status]);
...
//       const res = await fetch(`/api/barter-offers/${offer.offer_id}`, {
//         method: "PUT",
//         headers: {
//           "Content-Type": "application/json",
//           Authorization: `Bearer ${token}`,
...
//         toast({
//           title: "Error",
//           description: "Missing recipient user information.",
//           variant: "destructive",
//         });
//         return;
//       }
...
//       const threadId = data.thread?.thread_id ?? data.thread_id;
//       if (!threadId) throw new Error("Invalid thread data from server");
...
//     type === "sent"
//       ? `Owner #${offer.listing?.user_id ?? "?"}`
//       : offer.offerer?.username ?? "Unknown";
//   const otherUserAvatar =
//     type === "sent" ? undefined : offer.offerer?.avatar_url;
            ...
            <div>
              ...
              <div className="flex gap-3 p-3 bg-muted/50 rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <h4 className="font-medium text-sm">{offer.listing.title}</h4>
                </div>
              </div>
            </div>
          )}
...
<RatingDialog
  ...
  barter={{
    ...
    other_user_name:
      type === "sent"
        ? `Owner #${offer.listing?.user_id ?? "?"}`
        : offer.offerer?.username ?? "Unknown User",
    other_user_id:
      type === "sent"
        ? (offer.listing?.user_id ?? "").toString()
        : offer.offerer_id.toString(),
