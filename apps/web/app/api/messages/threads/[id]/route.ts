...
export async function GET(
  ...
  try {
    ...
    const threadId = Number(id);
    if (!Number.isFinite(threadId)) {
      return new NextResponse(JSON.stringify({ error: "Invalid thread id" }), {
        status: 400,
        headers: { "Cache-Control": "no-store" },
      });
    }
    ...
    const payload = {
      ...
      messages: thread.messages.map((m) => ({
        message_id: m.message_id,            // keep numeric for stability
        thread_id: m.thread_id,
        sender_id: String(m.sender_id),
        sender_name: m.sender?.username ?? "Anonymous",
        sender_avatar: m.sender?.avatar_url ?? null,
    ...
    return new NextResponse(JSON.stringify(payload), {
      status: 200,
      headers: { "Cache-Control": "no-store" }, // 💡 prevent stale reads
    });
  } catch (error) {
    console.error("❌ Get thread messages error:", error);
