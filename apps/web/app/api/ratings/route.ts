...
export async function POST(request: NextRequest) {
  ...
  console.log("Received rating payload:", await request.clone().json());


    const body = await request.json();
    const barter_id = Number(body.barter_id);
    const rated_user_id = Number(body.rated_user_id);
    const score = Number(body.score ?? body.rating);
    const comment = body.comment?.trim() || null;

    // ✅ Input validation
    if (!barter_id || !rated_user_id || !score) {
      return NextResponse.json(
        { error: "Missing required fields: barter_id, rated_user_id, or score" },
        { status: 400 }
      );
    }
    if (isNaN(barter_id) || isNaN(rated_user_id) || isNaN(score)) {
    ...
    // ✅ Allow rating if completed or recently accepted
    if (!["completed", "accepted"].includes(barter.status)) {
      return NextResponse.json(
        { error: `Barter not completed (status: ${barter.status})` },
        { status: 400 }
      );
    }
    ...
    // ✅ Ensure the rater is part of the barter
    if (
      barter.offerer_id !== currentUserId &&
      barter.listing.user_id !== currentUserId
    ) {
      return NextResponse.json(
        { error: "You are not a participant in this barter" },
        { status: 403 }
      );
    }
    ...
    if (rated_user_id !== expectedRatedUserId) {
      return NextResponse.json(
        { error: "Rated user ID does not match barter participants" },
        { status: 400 }
      );
    }
    ...
    // ✅ Optional: Recalculate target user’s average
    await prisma.user.update({
      where: { user_id: rated_user_id },
      data: {
        rating: {
          increment: 0, // trigger recalculation on future aggregation
        },
      },
    });

    return NextResponse.json(
      { message: "Rating created", rating: newRating },
      { status: 201 }
    );
  } catch (error) {
    console.error("Create rating error:", error);
