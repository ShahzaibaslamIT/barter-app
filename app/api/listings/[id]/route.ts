...
export async function GET(
  ...
  try {
    ...
    // 🛑 HARD GUARD (Fix 3)
    if (!id || id === "undefined") return invalidIdResponse();

    const itemId = Number(id);
    if (Number.isNaN(itemId)) return invalidIdResponse();
    ...
    console.error("[listing GET by ID] error:", error);
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    ...
    // 3️⃣ Update PostGIS geometry safely
    if (
      latitude !== undefined &&
      longitude !== undefined &&
